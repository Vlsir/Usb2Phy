...
from ..cmlro import Idac, Pbias
from .tb import Pvt, TbParams


# Widely reused parameter values
...
def codesweep(tbgen: h.Generator, pvt: Pvt) -> List[hs.SimResult]:
    ...
    params = [TbParams(pvt=pvt, cml=cml, code=code) for code in codes]
...
def run_corners(tbgen: h.Generator) -> Result:
    ...
    pickle.dump(asdict(result), open("idac.codesweep.pkl", "wb"))
...
def run_one() -> hs.SimResult:
    ...
    params = TbParams(pvt=Pvt(), cml=cml, code=16)
...
def test_idac_code_sweep():
    ...
    # result = Result(**pickle.load(open("idac.codesweep.pkl", "rb")))
